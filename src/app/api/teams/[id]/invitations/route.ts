...
export async function GET(_: NextRequest, context: RouteContext<'/api/teams/[id]/invitations'>) {
  ...
  const { id } = await context.params; if (!await authorize(session.uid, id)) return NextResponse.json({ error: 'Invite permission is required.' }, { status: 403 });
...
export async function POST(request: NextRequest, context: RouteContext<'/api/teams/[id]/invitations'>) {
  ...
  const { id } = await context.params; if (!await authorize(session.uid, id)) return NextResponse.json({ error: 'Invite permission is required.' }, { status: 403 });
  const parsed = z.object({ email: z.email().transform(normalizeEmail), canEdit: z.boolean(), canDelete: z.boolean(), canShare: z.boolean(), canInvite: z.boolean(), canManageAccess: z.boolean() }).safeParse(await request.json().catch(() => null));
